...
const {regTempalte} =require('../util/template')
...
const register = async (req,res) => {
    try{
        ...
        res.status(StatusCodes.OK).json({ msg: " New user Registered successfully", newUser, emailRes })
     ...
     let extUser = await user.findOne({ email })
        if(!extUser)
            ...
            let isMatch = await bcrypt.compare(password,extUser.password)
                if(!isMatch)
                    return res.status(StatusCodes.NOT_ACCEPTABLE).json({ msg: "Passwords are not match"})

                    //login token
                    const authToken = createAcessToken({ id: extUser._id})
...
const getCurrentUser = async (req,res) => {
    try{
        ...
        let user = await User.findById({ _id: userId }).select("_password")
